...
func (network *Network) handleRPC(msg *RPCMessage, addr *net.UDPAddr) {
	switch msg.Type {
	case "PING":
		if msg.Query {
			// Got PING → send back PONG
			...
			reply := NewRPCMessage("PING", replyPayload, msg.DestAddr, msg.SourceAddr, false)
